...
class Nova
{
    ...
    public static function resourceInformation(Request $request)
    {
        return static::resourceCollection()->map(function ($resource) use ($request) {
            return array_merge([
                ...
                'showUpload' => $resource::showUpload(),
                'additionalButtons' => $resource::additionalButtons(),
    ...
    public static function defaultFooter(Request $request)
    {
       $url =  $request->url();
        ...
        if( config('app.name') == 'WEPUSH' ) {
            $name = "WePushCalls";
        } else if(config('app.name') == 'VOCODIA') {
            $name = 'VOCODIA';
        }
        else {
            if ( strstr( $url, 'voslogic' ) ) {
                $name =  "Vos Logic";
            } else {
                $name =  "Callzy";
            }
        ...
        return Blade::render('
            <p class="text-center">Powered by <a class="link-default" href="#">'.$name.'</a></p>
