  ...
  >
    <template #field>
      ...
      <span v-if="shouldShowField" class="text-90 text-sm select-none">
        {{ (currentLabel.length > 20) ? currentLabel.substring(0,20) + '...' + currentLabel.substring(currentLabel.length - 5,currentLabel.length)  : currentLabel  }}
      </span>

      <p v-if="hasError" class="text-xs mt-2 text-red-500">{{ firstError }}</p>
      <p v-if="sizeError" class="text-xs mt-2 text-red-500">{{ firstSizeError }}</p>
    </template>
  </DefaultField>
</template>
...
export default {
  ...
  methods: {
    ...
    fileChange(event) {
      ...
      const fileSize = this.$refs.fileField.files[0].size / 1024 / 1024; //in mIb
      
      if (fileSize > 10) {
        this.sizeError = true
        this.firstSizeError = 'File size is greater than 10 Mb'
       this.filename = null
