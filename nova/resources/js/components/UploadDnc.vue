...
export default {
    ...
    methods:{
        submit(){
            ...
            Nova.request().post('/nova-api/upload/dnc-list', formdata, {headers}).then(res => {
            if(res.data.success){
                console.log('success')
                Nova.success('Dnc List Uploaded Successfully')
                Nova.visit('/resources/d-n-cs', { remote: false })
            }else{
                console.log('fail')
