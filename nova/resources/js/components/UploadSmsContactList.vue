...
export default {
    ...
    data: () => ({
        licenseKey: 'zRxFR9PTHuYTxDlha8IjQyQgRTA2V7',
        user: {
          user_id: 'default123',
          id: null,
          list_id: null,
    ...
    methods: {
        ...
        if(result){
            ...
            this.show = false;
            Nova.visit('/resources/sms-contact-lists', { remote: false })
            //custom code
        }else{
            console.log("fail");
            //custom code
        ...
        async submit() {
            await Nova.request().get('/nova-api/custom/sms-contact-list-create/'+this.contactListName)
            .then(response => {
                if(response.data){
