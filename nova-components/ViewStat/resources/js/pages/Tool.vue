  ...
  <div>
    ...
    <div class="flex" style="margin: 10px;">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow relative px-6 py-4 md:col-span-4 min-h-40 h-full flex-1">
        ...
        <div class="flex items-center mb-4">
          <div class="rounded-lg bg-primary-500 text-white h-14 w-14 flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="24" height="24" class="inline-block" role="presentation"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
          </div>
          <div class="ml-4">
            <p class="flex items-center text-4xl">{{avg_calls_duration}}</p>
            <div>
              <p class="flex items-center font-bold text-sm"><span class="text-gray-400 font-semibold"></span></p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="ml-auto bg-white dark:bg-gray-800 rounded-lg shadow relative px-6 py-4 md:col-span-4 min-h-40 h-full flex-1 ml-2 mr-2">
        <div class="h-6 flex items-center mb-4">
          <h3 class="mr-3 leading-tight text-sm font-bold">Call Back %</h3>
        ...
        <div class="flex items-center mb-4">
          ...
          <div class="ml-4">
            <p class="flex items-center text-4xl">{{call_back_percentage}} %</p>
            <div>
              <p class="flex items-center font-bold text-sm"><span class="text-gray-400 font-semibold"></span></p>
            </div>
          </div>
      ...
      <div class="ml-auto bg-white dark:bg-gray-800 rounded-lg shadow relative px-6 py-4 md:col-span-4 min-h-40 h-full flex-1">
        <div class="h-6 flex items-center mb-4">
          <h3 class="mr-3 leading-tight text-sm font-bold">Number Of Call Backs</h3>
        ...
        <div class="flex items-center mb-4">
          ...
          <div class="ml-4">
            <p class="flex items-center text-4xl">{{call_backs}}</p>
            <div>
              <p class="flex items-center font-bold text-sm"><span class="text-gray-400 font-semibold"></span></p>
            </div>
          </div>
    ...
    <div>
      <div class="flex justify-between">
          <div class="flex-1" style="margin: 10px;">
              <div v-if="campaign.campaign_stats && campaign.campaign_stats.sent_count">
                  Sent:
                  {{campaign.campaign_stats.sent_count +' '+ '('+ ((campaign.campaign_stats.sent_count) / parseFloat(campaign.campaign_stats.contact_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '') + ')' }}
              </div>
              <div v-else>
                  Sent: 0 (0 %)
              </div>
          </div>
          <div class="flex-1" style="margin: 10px;" v-if="pending !== 0">
              Pending:
              {{pending +' '+  '('+( pending / campaign.campaign_stats.contact_count * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)' }}
          </div>
          <div v-else class="flex-1" style="margin: 10px;">
              Pending: 0 (0 %)
          </div>
      </div>
      <div class="flex justify-between">
          <div class="flex-1" style="margin: 10px;">
            Status:
            <span v-if="campaign.status == 'inactive'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold badge-danger">{{campaign.status}}</span>
            <span v-if="campaign.status == 'pending'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold badge-info">{{campaign.status}}</span>
            <span v-if="campaign.status == 'paused'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold badge-warning">{{campaign.status}}</span>
            <span v-if="campaign.status == 'finished'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold badge-finished">{{campaign.status}}</span>
            <span v-if="campaign.status == 'played'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold bg-green-100 text-green-600 dark:bg-green-500 dark:text-green-900">{{campaign.status}}</span>
            <span v-if="campaign.status == 'outside of hours'" class="inline-flex items-center whitespace-nowrap h-6 px-2 rounded-full uppercase text-xs font-bold badge-warning">{{campaign.status}}</span>
          </div>

          <div class="flex-1" style="margin: 10px;">
              <div v-if="campaign.campaign_stats">
                  DNC Calls:
                  {{ campaign.campaign_stats.dnc_count +' '+  '('+(parseFloat(campaign.campaign_stats.dnc_count) / parseFloat(campaign.campaign_stats.contact_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)'}}
              </div>
              <div v-else>
                  DNC Calls: 0 (0 %)
              </div>
      ...
      <div class="flex justify-between">
        <div class="flex-1" style="margin: 10px;" v-if="campaign.campaign_type == 'press-1'">
              <div v-if="optOut_count != 0 && $ivr_count != 0">
                  OptOut Count:
                  {{ optOut_count +' '+  '('+(parseFloat(optOut_count) / parseFloat(ivr_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)'}}
              </div>
              <div v-else>
                  Optout Count: 0 (0 %)
              </div>
        ...
        <div class="flex-1" style="margin: 10px;" v-if="campaign.campaign_type == 'press-1'">
              <div v-if="optIn_count != 0 && $ivr_count != 0">
                  OptIn Count:
                  {{ optIn_count +' '+  '('+(parseFloat(optIn_count) / parseFloat(ivr_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)'}}
              </div>
              <div v-else>
                  OptIn Count: 0 (0 %)
              </div>
      ...
      <div class="flex justify-between">
        <div class="flex-1" style="margin: 10px;" v-if="campaign.campaign_type == 'press-1'">
              <div v-if="vm_count != 0 && $ivr_count != 0">
                  VM Count:
                  {{ vm_count +' '+  '('+(parseFloat(vm_count) / parseFloat(ivr_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)'}}
              </div>
              <div v-else>
                  VM Count: 0 (0 %)
              </div>
        ...
        <!-- <div class="flex-1" style="margin: 10px;" v-if="campaign.campaign_type != 'press-1'">
            <div v-if="campaign.campaign_stats && campaign.campaign_stats.success_count">
                Transfer:
                {{ campaign.campaign_stats.success_count +' '+ '('+ (parseFloat(campaign.campaign_stats.success_count) / parseFloat(campaign.campaign_stats.contact_count) * 100 ).toFixed(2).replace(/(\.0+|0+)$/, '')+ '%)'}}
            </div>
            <div v-else>
                Transfer: 0 (0 %)
            </div>
