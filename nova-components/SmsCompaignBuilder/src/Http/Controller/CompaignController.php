...
namespace Sms\SmsCompaignBuilder\Http\Controller;
...
use App\Jobs\SmsCampaignJobStart;
use App\Models\Company;
use App\Models\SwSmsCampaignLog;
...
class CompaignController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $sw_camapign_logs = new SwSmsCampaignLog;
        $sw_camapign_logs->user_id = $user->id;
        $sw_camapign_logs->company_id = $company->id;
        $sw_camapign_logs->sms_campaign_id = $campaign->id;
        $sw_camapign_logs->request_data = $request_params;
        $sw_camapign_logs->response_data = $response;
        $sw_camapign_logs->save();

        if (!property_exists($json_response, 'errors')) {
            $sms_camp = SmsCampaign::Find($campaign_id);
            $sms_camp->sw_campaign_id = $json_response->id;
            $sms_camp->status = 'preprocessing';
            $sms_camp->save();
        }
