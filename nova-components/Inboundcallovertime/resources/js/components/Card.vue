...
export default {
    ...
    methods: {
        async getData(start_date, end_date) {
            ...
            let baseUrl = "/nova-api/custom/get-inbound-call";
            return await Nova.request()
                .post(baseUrl, params)
                .then((response) => {
                    let inboundCall = response.data.query.inboundCall;
                    let dateRange = response.data.query.inboundCall;
                    ...
                    console.log("16.inboundcallovertime ---->", response.data.query);
                });
        },
        ...
        buildChart() {
            ...
            chart.colors.list = [
                 window.Nova.config('brandColors')['500-hex'],
            ]
