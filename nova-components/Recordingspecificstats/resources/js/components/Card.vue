    ...
    <loading-card :loading="loading" class="flex flex-col items-center justify-center">
        <div class="px-3 py-3">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow mb-6 mt-6">
                ...
                <tbody  v-if="recordingStats.length>0">
                    <tr
                        v-for="item in recordingStats"
                        :key="item.name"
                        dusk="53-row"
                        class="group"
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <p>
                                    {{
                                        item.avg_call_backs !== 0 &&
                                        item.avg_call_backs !== null
                                            ? item.calls_back_count !== null
                                                ? (
                                                        (parseFloat(
...
export default {
    ...
    mounted() {
        ...
        // console.log("recordingStats", query.recordingStats);

        // this.recordingStats = query.recordingStats;
        let start_date = "";
        let end_date = "";
    ...
    methods: {
        ...
        async getData(start_date, end_date) {
            ...
            let baseUrl = "/nova-api/custom/get-recording-stats";
            await Nova.request()
                .get(baseUrl, params)
                .then((response) => {
                    // if (response.data.query.length > 0) {
                    this.recordingStats = response.data.query.data;
