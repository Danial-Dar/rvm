...
class Email extends Resource
{
    ...
    public static function additionalButtons()
    {
        ...
        if($user && $user->can('upload-dnc')) {
            return [[
                 //'component' => 'UploadEmail'
                'component' => 'UploadEmailCsvBox'
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ...
            /*PhoneNumber::make('Email', 'email')->sortable()->rules('required', function($attribute, $value, $fail) {
                // if (strtoupper($value) !== $value) {
                //     return $fail('The '.$attribute.' field must be uppercase.');
                // }
                $raw_email = preg_replace("/^[a-zA-Z-' ]*$/", '', $value);

                if (strlen($raw_email) <= 10  || strlen($raw_email) < 11) {
