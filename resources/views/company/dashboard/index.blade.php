...
<div class="contents">
      ...
      <div class="container-fluid">
         ...
         <div class="row ">
               ...
               <div class="ap-po-details ap-po-details--2 p-25 radius-xl bg-white d-flex justify-content-between">
                  <div>
                     <div class="overview-content">
                        <h1 id="count_campaigns">{{$count_campaigns}}</h1>
                        <p>Total Campaigns</p>
                     </div>
                  </div>
            ...
            <div class="col-xxl-3 col-sm-3  col-ssm-6 mb-30">
               ...
               <div class="ap-po-details ap-po-details--2 p-25 radius-xl bg-white d-flex justify-content-between">
                  <div>
                     <div class="overview-content">
                        <h1 id="count_contact_lists">{{$count_contact_lists}}</h1>
        ...
        <div class="row">
            <div class="col-lg-4 mb-20">
                <select name="user-select" id="user-select" class="form-control" onchange="showCampaignsRelatedToUser(this.value)">
            ...
            <div class="col-lg-4 mb-20" id="campaignDropdown" style="display:none;">
            ...
            <div class="col-lg-4 mb-20" id="campaignDateRange" style="display:none;">
  ...
  {{-- content end --}}
    ...
    @if(auth()->user()->role !== "company")
        <input type="hidden" id="dashboardResultURL" value="{{url('user/dashboard/results/')}}">
        <input type="hidden" id="ajaxDashboardURL" value="{{url('user/ajax_dashboard')}}">
        <input type="hidden" id="dashboardPerHourURL" value="{{url('user/dashboard/per_hour_results')}}">
        
    @else
       
        <input type="hidden" id="dashboardResultURL" value="{{url('company/dashboard/results/')}}">
        <input type="hidden" id="ajaxDashboardURL" value="{{url('company/ajax_dashboard')}}">
        <input type="hidden" id="dashboardPerHourURL" value="{{url('company/dashboard/per_hour_results')}}">
    @endif
  <script>
    $(function() {
...
<script>
    ...
    function showCampaignsRelatedToUser(value){
        if(value !== ""){

            $('#campaignDropdown').show();
            $('#campaignDateRange').show();

            var inside_url = 'company/user_campaigns/'+value; 
            var url = '{{url('')}}/'+inside_url;
            $('#camapign-select').empty();
            $.ajax({
                url: url,
                contentType: "application/json; charset=utf-8", 
                success: function(result){
               
                    if(result['user_campaigns'].length > 0){
                        $('#camapign-select').append('<option value=""Selected> Choose campaign</option>')
                        $.each(result['user_campaigns'], function (i, item) {
                            $('#camapign-select').append($('<option>', { 
                                value: item.id,
                                text : item.name 
                            }));
                        });
                    }else{
                        flash( 'No campaign available.',{
                            // background color
                            'bgColor' : '#C0392B',
                        ...
                        $('#campaignDropdown').hide();
                        $('#campaignDateRange').hide();
                        document.getElementById("content_first").style.display = 'none';
                        document.getElementById("content_third").style.display = 'none';
                        document.getElementById("content_fourth").style.display = 'none';
                        document.getElementById("content_fifth").style.display = 'none';
                        document.getElementById("dropsPerHourDailyDatePicker").style.display = 'none';
   ...
   async function showHideGraphs(start_date,end_date) {
      ...
      var x = element.value;
      var url = $('#dashboardResultURL').val();
      // alert(url+'/'+x);
   ...
   function graphs(list_names, list_contacts,successful, initiated, pending, fail,
   sent,totalCampaignContacts,campaignInitiatedPerHour,periods,totalCampaignInitiated,
   dropsPerHourArray) 
   {
  ...
  function campaignInitiatedPerHour(date){
    ...
    var route = $('#dashboardPerHourURL').val();
